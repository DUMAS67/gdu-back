...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public UserDetailsService userDetailsService(DataSource ds) {
		...
		manager.setUsersByUsernameQuery("select email, mot_de_passe, 'true' from collaborateur where email=?");
		manager.setAuthoritiesByUsernameQuery(
				"select c.email, rc.role from collaborateur c, role_collaborateur rc where c.id=rc.collaborateur_id and c.email=?");
		return manager;
	}
